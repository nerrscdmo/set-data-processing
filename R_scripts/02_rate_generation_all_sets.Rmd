...
for(i in seq_along(reserves)){
    ...
    # general details of SET landscapes and settings
    ...
    set_slr_rates <- res_set_details |> 
        ...
        summarize(.by = c(reserve, slr_rate, slr_CI_low, slr_CI_high,
               slr_19yr, yr19_CI_low, yr19_CI_high),  
               n = n()) |> 
        mutate(longterm = glue::glue("{slr_rate} mm/yr ({slr_CI_low} - {slr_CI_high})"),
               yr19 = glue::glue("{slr_19yr} mm/yr ({yr19_CI_low} - {yr19_CI_high})")) |> 
    select(reserve, n, slr_rate, slr_19yr, longterm, yr19) |> 
    pivot_longer(c(longterm, yr19),
                 names_to = "comp",
                 values_to = "Rate") |> 
    mutate(comp = case_when(comp == "longterm" ~ "Long-term",
                            comp == "yr19" ~ "Near-term"),
           Water_Rate = case_when(comp == "Long-term" ~ slr_rate,
                                  comp == "Near-term" ~ slr_19yr)) |> 
        rename(Water_Comparison = comp) |> 
        select(reserve, Water_Comparison, Water_Rate, Rate, n)
    
    # summary of comparisons
    slr_comps <- res_set_details |> 
        summarize(.by = c(dir_slr, slr_rate),
                  n = n()) |> 
        rename(Result = dir_slr,
               Water_Rate = slr_rate) |> 
        mutate(Water_Comparison = "Long-term")
    
    yr19_comps <- res_set_details |> 
        summarize(.by = c(dir_19yr, slr_19yr),
                  n = n()) |> 
        rename(Result = dir_19yr,
               Water_Rate = slr_19yr) |> 
        mutate(Water_Comparison = "Near-term")
    
    all_comps <- bind_rows(slr_comps, yr19_comps)
    
    comps_wide_withRates <- all_comps |> 
        arrange(desc(Result)) |> 
        pivot_wider(names_from = Result,
                    values_from = n,
                    values_fill = 0) |> 
        as.data.frame() |> 
        left_join(set_slr_rates) |> 
        relocate(Water_Comparison, Rate) |> 
        select(-reserve, -Water_Rate) |> 
        rename(Total = n)
    ...
    res_set_list <- list(
        ...
        types = set_types,
        vegs = set_vegs,
        water_comp_rates = set_slr_rates,
        water_comp_summaries = comps_wide_withRates,
